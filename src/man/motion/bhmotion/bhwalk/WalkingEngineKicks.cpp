...
void WalkingEngineKick::getPreStepSize(float& rotation, Vector3BH<>& translation) const
{
...
void WalkingEngineKick::getStepSize(float& rotation, Vector3BH<>& translation) const
{
...
void WalkingEngineKicks::load(const std::string &configDir)
{
    ...
    for(int i = 0; i < (WalkRequest::numOfKickTypes - 1) / 2; ++i)
    {
        ...
        std::cout << "WalkingEngineKicks Load File: " << path.c_str() << std::endl;
    }
}

void WalkingEngineKicks::load(const std::string &configDir, WalkRequest::KickType type, char* data)
{
        std::string path = configDir + "Kicks/" + WalkRequest::getName(WalkRequest::KickType(type)) + ".cfg";
        if(!kicks[(type - 1) / 2].load(path.c_str()))
    return;
        std::cout << "WalkingEngineKicks Load: " << WalkRequest::getName(WalkRequest::KickType(type)) << std::endl;
        OUTPUT(idText, text, filePath << ": ok");
}

void WalkingEngineKicks::getKickStepSize(WalkRequest::KickType type, float& rotation, Vector3BH<>& translation) const
{
...
void WalkingEngineKicks::getKickPreStepSize(WalkRequest::KickType type, float& rotation, Vector3BH<>& translation) const
{
    bool mirrored = (type - 1) % 2 != 0;
    const WalkingEngineKick& kick = kicks[(type - 1) / 2];
...
void WalkingEngineKickPlayer::applyHeadAndArms(float headJointAngles[2], float leftArmJointAngles[4], float rightArmJointAngles[4])
{
    ...
    for(int i = 0; i < 2; ++i)
        if(headJointAngles[i] != JointDataBH::off)
