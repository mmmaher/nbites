...
void KickEngineData::calculateOrigins(const RobotDimensionsBH& theRobotDimensionsBH, const FilteredJointDataBH& jd)
{
    if(!wasActive)
    {
        origins[Phase::leftFootTra] = robotModel.limbs[MassCalibrationBH::footLeft].translate(0.f, 0.f, -theRobotDimensionsBH.heightLeg5Joint).translation;
        origins[Phase::rightFootTra] = robotModel.limbs[MassCalibrationBH::footRight].translate(0.f, 0.f, -theRobotDimensionsBH.heightLeg5Joint).translation;
...
bool KickEngineData::checkPhaseTime(const FrameInfoBH& frame, const RobotDimensionsBH& rd, const FilteredJointDataBH& jd, const TorsoMatrixBH& torsoMatrix)
{
    ...
    if(motionID > -1)
    {
        ...
        else if(currentParameters.loop && phaseNumber == currentParameters.numberOfPhases)
        {
            ...
            currentParameters.initFirstPhaseLoop(origins, currentParameters.phaseParameters[currentParameters.numberOfPhases - 1].comTra[2], Vector2<>(jd.angles[JointDataBH::HeadPitch], jd.angles[JointDataBH::HeadYaw]));
            if(currentKickRequest.dynamical && !currentKickRequest.dynPoints.empty())
                for(unsigned int i = 0; i < currentKickRequest.dynPoints.size(); i++)
                    if(currentKickRequest.dynPoints[i].phaseNumber == phaseNumber)
                        addDynPoint(currentKickRequest.dynPoints[i], rd, torsoMatrix);
...
bool KickEngineData::calcJoints(JointRequestBH& jointRequest, const RobotDimensionsBH& rd, const HeadJointRequest& hr)
{
    //Calculate Legs
    if(motionID > -1)
    {
...
void KickEngineData::calcLegJoints(const JointDataBH::Joint& joint, JointRequestBH& jointRequest, const RobotDimensionsBH& theRobotDimensionsBH)
{
    ...
    anklePos -= Vector3<>(0.f, sign * (theRobotDimensionsBH.lengthBetweenLegs / 2), -theRobotDimensionsBH.heightLeg5Joint);
...
void KickEngineData::simpleCalcArmJoints(const JointDataBH::Joint& joint, JointRequestBH& jointRequest, const RobotDimensionsBH& theRobotDimensionsBH, const Vector3<>& armPos, const Vector3<>& handRotAng)
{
    float sign = joint == JointDataBH::LShoulderPitch ? 1.f : -1.f;

    Vector3<> target =  armPos -  Vector3<>(theRobotDimensionsBH.armOffset.x,
    ...
    float length2ElbowJoint = Vector3<>(theRobotDimensionsBH.upperArmLength, theRobotDimensionsBH.yElbowShoulder, 0.f).abs();
...
void KickEngineData::calculatePreviewCom(Vector3<>& ref, Vector2<>& origin)
{
...
void KickEngineData::getCOMReference(const Vector3<>& lFootPos, const Vector3<>& rFootPos, Vector3<>& comRef, Vector2<>& origin)
{
...
void KickEngineData::setStaticReference()
{
    if(!currentParameters.autoComTra)
    {
        Vector2<> staticref =  currentParameters.getComRefPosition(phase, phaseNumber);
...
void KickEngineData::transferDynPoint(Vector3<>& d, const RobotDimensionsBH& rd, const TorsoMatrixBH& torsoMatrix)
{
...
void KickEngineData::setRobotModel(RobotModel rm)
{
    robotModel = rm;
}
...
void KickEngineData::initData(const bool& compensated, const FrameInfoBH& frame, const MotionRequest& mr, const RobotDimensionsBH& theRobotDimensionsBH, std::vector<KickEngineParameters>& params, const FilteredJointDataBH& jd, const TorsoMatrixBH& torsoMatrix)
{
    if(getMotionIDByName(mr, params))
    {
        ...
        currentParameters.initFirstPhase(origins, Vector2<>(jd.angles[JointDataBH::HeadPitch], (mr.kickRequest.mirror) ? -jd.angles[JointDataBH::HeadYaw] : jd.angles[JointDataBH::HeadYaw]));
        ...
        if(currentKickRequest.dynamical && !currentKickRequest.dynPoints.empty())
            for(unsigned int i = 0; i < currentKickRequest.dynPoints.size(); i++)
                if(currentKickRequest.dynPoints[i].phaseNumber == phaseNumber)
                    addDynPoint(currentKickRequest.dynPoints[i], theRobotDimensionsBH, torsoMatrix);
    }
}
...
bool KickEngineData::sitOutTransitionDisturbance(bool& compensate, bool& compensated, const FilteredSensorDataBH& sd, KickEngineOutputBH& kickEngineOutput, const JointData& jd, const FrameInfoBH& frame)
{
    if(compensate)
    {
        if(!startComp)
