...
public:
    ...
    void calculateOrigins(const RobotDimensionsBH& theRobotDimensionBH, const FilteredJointDataBH& jd);
    ...
    void initData(const bool& compensated, const FrameInfoBH& frame, const MotionRequestBH& mr, const RobotDimensionsBH& theRobotDimensionsBH, std::vector<KickEngineParameters>& params, const FilteredJointDataBH& jd, const TorsoMatrixBH& torsoMatrix);
    void setEngineActivation(const float& ratio);
    bool activateNewMotion(const KickRequest& br, const bool& isLeavingPossible);
    bool sitOutTransitionDisturbance(bool& compensate, bool& compensated, const FilteredSensorDataBH& sd, KickEngineOutputBH& kickEngineOutput, const JointDataBH& jd, const FrameInfoBH& frame);
