/**
 * @file Vector2.h
 * Contains template class Vector2BH of type V
 *
...
template <class V = float> class Vector2BH : public ImplicitlyStreamable
{
public:
  /** The vector values */
  V x, y;
  ...
  /** Default constructor. */
  inline Vector2BH<V>() : x(V()) , y(V()) {}

  /** Constructor. */
  inline Vector2BH<V>(V x, V y) : x(x), y(y) {}

  /** Copy constructor
  *\param other The other vector that is copied to this one
  */
  inline Vector2BH<V>(const Vector2BH<V>& other) : x(other.x), y(other.y) {}

  /** Copy constructor from different element type
   *\param O The type of other's elements
   *\param other The other vector that is copied to this one
   */
  template<typename O> inline explicit Vector2BH<V>(const Vector2BH<O>& other) : x((V) other.x), y((V) other.y) {}

  /** Assignment operator
  *\param other The other vector that is assigned to this one
  *\return A reference to this object after the assignment.
  */
  inline Vector2BH<V>& operator=(const Vector2BH<V>& other)
  {
    x = other.x;
    y = other.y;
    return *this;
  ...
  *\return A reference to this object after the calculation.
  */
  inline Vector2BH<V>& operator*=(const V& factor)
  {
    x *= factor;
    y *= factor;
    return *this;
  ...
  */
  inline Vector2BH<V> operator+(const Vector2BH<V>& other) const
  {
    return Vector2BH<V>(*this) += other;
  }
  ...
  *\return A new object that contains the result of the calculation.
  */
  inline Vector2BH<V> operator-() const
  {
    return Vector2BH<V>(-x, -y);
  }
  ...
  */
  inline V operator*(const Vector2BH<V>& other) const
  {
  ...
  *\return A new object that contains the result of the calculation.
  */
  inline Vector2BH<V> operator*(const V& factor) const
  {
    return Vector2BH<V>(*this) *= factor;
  }
  ...
  */
  inline bool operator==(const Vector2BH<V>& other) const
  {
    return x == other.x && y == other.y;
  }
  ...
  *\return the rotated vector.
  */
  Vector2BH<V>& rotate(float alpha)
  {
...
/**
* Streaming operator that writes a Vector2BH to a stream.
* @param out The stream to write on.
* @param vector The Vector2BH object.
* @return The stream.
*/
template <class T>
Out& operator<<(Out& out, const Vector2BH<T>& vector)
{
...
/**
* Streaming operator that reads a Vector2BH from a stream.
* @param in The stream from which is read.
* @param vector The Vector2BH object.
* @return The stream.
*/
template <class T>
In& operator>>(In& in, Vector2BH<T>& vector)
{
  STREAM_REGISTER_BEGIN_EXT(vector);
