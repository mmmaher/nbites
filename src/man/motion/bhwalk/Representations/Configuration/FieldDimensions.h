...
* @author Max Risler
*/
class FieldDimensionsBH : public Boundary<>, public SimpleFieldDimensions
{
public:
  /**
  * This is a collection of line- or boundary segments with start-Pose2DBH and length.
  */
  class LinesTable
  {
  public:
    ...
    public:
      Pose2DBH corner; /**< The field corners. */
    ...
    STREAMABLE(Circle,
    {,
      (Vector2BH<>) center, /**< The center of the circle. */
    ...
    void push(const Pose2DBH& p, float l, bool isPartOfCircle = false);

    void push(const Vector2BH<>& s, const Vector2BH<>& e, bool isPartOfCircle = false);

    void pushCircle(const Vector2BH<>& center, float radius, int numOfSegments);

    /**
    * Get the the closest point to p on a field line
    */
    Vector2BH<> getClosestPoint(const Vector2BH<>& p) const;
    ...
    */
    bool getClosestIntersection(const Geometry::Line& l, int& outLineIndex, Vector2BH<>& outIntersection) const;

    bool getClosestIntersection(const Geometry::Line& l, Vector2BH<>& outIntersection) const;
    ...
    */
    bool isInside(const Vector2BH<>& v) const;
    ...
    */
    bool getClosestPoint(Vector2BH<>& point, const Pose2DBH& p, int numberOfRotations, float minLength) const;
    ...
    */
    float getDistance(const Pose2DBH& pose) const;
  };
  ...
  */
  class CornersTable : public std::vector<Vector2BH<> >
  {
  public:
    /**
     ...
     */
    const Vector2BH<>& getClosest(const Vector2BH<>& p) const;
    ...
    * @return The position of the closest corner.
    */
    Vector2BH<int> getClosest(const Vector2BH<int>& p) const;
  };
  ...
  */
  bool isInsideCarpet(const Vector2BH<> &p) const
  {
  ...
  * @return How far was the point moved?
  */
  float clipToCarpet(Vector2BH<>& v) const
  {
  ...
  */
  bool isInsideField(const Vector2BH<> &p) const
  {
  ...
  * @return How far was the point moved?
  */
  float clipToField(Vector2BH<>& v) const
  {
  ...
  */
  Pose2DBH randomPoseOnField() const;

  /**
  * The function returns a random pose on the carpet.
  * @return The random pose.
  */
  Pose2DBH randomPoseOnCarpet() const;
