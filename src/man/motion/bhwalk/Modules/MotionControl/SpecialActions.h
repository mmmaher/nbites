...
private:
  ...
  */
  ...
  public:
    ...
    float d[JointRequestBH::numOfJoints + 4]; /**< Represent a set of values from a data line. */
    /*
    //possible content:
    ...
    */
    ...
    void toJointRequest(JointRequestBH& jointRequest, int& dataRepetitionCounter, bool& interpolationMode, bool& deShakeMode) const
    {
      for(int i = 0; i < JointRequestBH::numOfJoints; ++i)
        jointRequest.angles[i] = (float) d[i + 1];
      interpolationMode = (int(d[JointRequestBH::numOfJoints + 1]) & 1) != 0;
      deShakeMode = (int(d[JointRequestBH::numOfJoints + 1]) & 2) != 0;
      dataRepetitionCounter = int(d[JointRequestBH::numOfJoints + 2]);
    }
    ...
    void toHardnessRequest(HardnessData& hardnessRequest, int& hardnessInterpolationTime)
    {
      ...
      hardnessInterpolationTime = int(d[JointRequestBH::numOfJoints + 1]);
    }

    SpecialActionRequest::SpecialActionID getSpecialActionID() const
    {
      return SpecialActionRequest::SpecialActionID(short(d[JointRequestBH::numOfJoints + 3]));
    }
  };
  ...
  */
  ...
  public:
    ...
    */
    ...
    Pose2DBH odometryOffset; /**< The displacement performed by the special action. */
  ...
  JointRequestBH currentRequest, /**< Current joint data. */
  ...
  */
  ...
  bool getNextData(const SpecialActionRequest& specialActionRequest, SpecialActionsOutputBH& specialActionsOutput);

  /** Calculates the next joint data vector by interpolating if necessary */
  void calculateJointRequest(JointRequestBH& jointRequest);

  void update(SpecialActionsOutputBH& specialActionsOutput);
