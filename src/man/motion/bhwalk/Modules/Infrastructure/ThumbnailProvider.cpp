  ...
  if(grayscale)
   {
    ...
    {
      case 8:
        shrinkGrayscale8x8SSE(theImageBH, thumbnail.imageGrayscale);
        break;
      case 4:
        shrinkGrayscale4x4SSE(theImageBH, thumbnail.imageGrayscale);
        break;
      default:
        shrinkGrayscaleNxN(theImageBH, thumbnail.imageGrayscale);
    }
  }
  else
  {
...
void ThumbnailProvider::shrinkNxN(const ImageBH& srcImage, ThumbnailBH::ThumbnailImage& destImage)
{
  ...
  for(int y = 0; y < height; ++y)
  {
    ...
    if(y % scaleFactor == scaleFactor - 1)
    {
      ...
      for(int i = 0; i < destImage.width; ++i, ++pSumms, ++pDest)
      {
        ...
        const short sumY = ptr[offsetof(ImageBH::Pixel, y)] + ptr[offsetof(ImageBH::Pixel, y) + sizeof(ImageBH::Pixel)];
        const short sumCb = ptr[offsetof(ImageBH::Pixel, cb)] + ptr[offsetof(ImageBH::Pixel, cb) + sizeof(ImageBH::Pixel)];
        const short sumCr = ptr[offsetof(ImageBH::Pixel, cr)] + ptr[offsetof(ImageBH::Pixel, cr) + sizeof(ImageBH::Pixel)];
...
void ThumbnailProvider::shrinkGrayscaleNxN(const ImageBH& srcImage, ThumbnailBH::ThumbnailImageGrayscale& destImage)
{
  const int scaleFactor = static_cast<int>(::pow(2.0, downScales));;
  const int averagedPixels = scaleFactor * scaleFactor;
  const int width = srcImage.width;
  ...
  const ImageBH::Pixel* pSrc;
  ThumbnailBH::ThumbnailImageGrayscale::PixelType* pDest;
  unsigned int* pSumms;
...
void ThumbnailProvider::shrinkGrayscale8x8SSE(const ImageBH& srcImage, ThumbnailBH::ThumbnailImageGrayscale& destImage)
{
  union
  {
    __m128i a;
  ...
  const unsigned char offset = offsetof(ImageBH::Pixel, y);
...
void ThumbnailProvider::shrinkGrayscale4x4SSE(const ImageBH& srcImage, ThumbnailBH::ThumbnailImageGrayscale& destImage)
{
  union
  {
    __m128i a;
