...
public:
    enum Track
    {
        headYaw = JointDataBH::HeadYaw,
        headPitch = JointDataBH::HeadPitch,
        lShoulderPitch = JointDataBH::LShoulderPitch,
        lShoulderRoll = JointDataBH::LShoulderRoll,
        lElbowYaw = JointDataBH::LElbowYaw,
        lElbowRoll = JointDataBH::LElbowRoll,
        rShoulderPitch = JointDataBH::RShoulderPitch,
        rShoulderRoll = JointDataBH::RShoulderRoll,
        rElbowYaw = JointDataBH::RElbowYaw,
        rElbowRoll = JointDataBH::RElbowRoll,
        numOfJointTracks,
...
public:
    ...
    void load(const std::string &configDir, WalkRequest::KickType type, char* data);

    bool isKickMirrored(WalkRequest::KickType type) const {return (type - 1) % 2 != 0;}
    void getKickStepSize(WalkRequest::KickType type, float& rotation, Vector3BH<>& translation) const;
    void getKickPreStepSize(WalkRequest::KickType type, float& rotation, Vector3BH<>& translation) const;
...
public:
    ...
    */
    ...
    void applyFoot(Pose3DBH& leftOriginToFoot, Pose3DBH& rightOriginToFoot);
