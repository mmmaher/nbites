"""
Game controller states for pDropIn, our player used for Drop-In games.
"""
...
def gameInitial(player):
    ...
    if player.firstFrame():
        ...
        player.stand()
        player.zeroHeads()
        #Reset localization to where the chaser would normally go
        player.brain.resetDropInInitialLocalization()
...
def gameReady(player):
    ...
    if player.firstFrame():
        ...
        # why is this different from player.stand() used in gameInitial?
        player.brain.tracker.repeatWidePan()
        player.timeReadyBegan = player.brain.time

        # Reset localization to where the chaser would normally be placed
...
def gameSet(player):
    ...
    # Wait until the sensors are calibrated before moving.
    # can this be incorporated into @ifGoNow or whatever was next to superState
    if not player.brain.motion.calibrated:
        return player.stay()

    #is the above statement necessary then?
    return player.stay()
