...
void TranscriberModule::processImage(messages::YUVImage image, std::string vtype) {
    ++image_index;
    ...
    SExpr inerts("InertialState", vtype, clock(), image_index, is_buf.length());
    ...
    SExpr joints("JointAngles", vtype, clock(), image_index, ja_buf.length());
